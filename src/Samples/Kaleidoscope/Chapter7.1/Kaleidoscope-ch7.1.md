...
In the previous chapters the code generation took an AST, converted it to LLVM IR, handed
...
As with many things in software, there are trade-offs involved. In this case the trade-off
...
The approach to balancing the trade-offs taken in this chapter is to eagerly compile top
...
The general idea is that the language runtime registers every lazy JIT function with the JIT
...
things in the JIT:
 ...
 2. Creates a stub implementation function in native code that will call back to the JIT
    when application code calls the function.
...
the native code, for the function.
...
### Initialization
The LLVM ORC JIT v2 uses a multi-layered system for materializing the IR and eventually the
...
generator. These are setup in the constructor and destroyed in the Dispose method.
...
### Body implementation
Since the lazy JIT registers the callback stub with the function's name when the actual
...
is left as an exercise for the reader :nerd_face:)

[!code-csharp[CloneAndRenameFunction](CodeGenerator.cs#CloneAndRenameFunction)]

The name used for the body is the original function name plus the suffix `$impl` tacked onto
the end. This suffix was chosen as it includes characters not allowed within the
Kaleidoscope language so there is no possibility of a name collision.

### Code generation
The next requirement is to change how we generate the functions. For an anonymous function
...
Function definitions for lazy JIT are first cloned and renamed, as discussed previously.
...
called to generate a function into a module and doesn't need to care if it is eager or lazy)
...
The JIT implementation will do the following after the generator callback returns:
 ...
 4. Update the stub for the function with the address of the function instead of the
    internal callback.
 5. return the address to the JIT engine so it can ultimately call the function and continue
    on it's merry way.

#### Lazy Materializer
The bulk of the work is in the ORCJIT v2 implementation however kaleidoscope must "hook"
...
by processing the AST into a module and providing that to the JIT.
...
occasionally get in the way - See [Special notes for interactive run-times](#special-notes-for-interactive-run-times)
below for more details)

### Special notes for interactive run-times
It turns out that re-definition of a lazy JIT'd function is a rather complex problem
