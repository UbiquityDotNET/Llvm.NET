...
function body is materialized through a delegate that will parse the LLVM IR for the body to
produce the required module. It then "emits" that module to the JIT engine before returning.
...
requirement. (It helps to keep the sample as simple as possible without crowding it with
parsing and other language specific cruft. For an example using a custom language AST see
