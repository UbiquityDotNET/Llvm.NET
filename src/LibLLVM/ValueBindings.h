...
extern "C" {
    ...
    */
    ...
    // This is optional but may be used by garbage collected runtimes to unprotect
    // the object the handle represents to it is collectable or decrement a ref count
    // for refcounted types, etc...
    // These call backs *MUST NOT* access the cache itself in any way. The actual
    // update to the cache itself has not yet occured so the cache won't reflect
    // the the end state of the update operation
