...
namespace Ubiquity.NET.Llvm.Interop
{
    ...
    /// <para>This interface is used to require types to implement the members. However, since
    /// it is only used for marshalling and the concrete type <typeparamref name="THandle"/>
    /// is known at such a call site. Thus, this marshalling occurs without boxing to this
    /// interface.</para>
    /// </remarks>
    internal interface IContextHandle<THandle>
        where THandle : struct, IContextHandle<THandle>
    {
        ...
        {
            if(DangerousGetHandle() == nint.Zero)
            {
        ...
        bool IsNull => DangerousGetHandle() == nint.Zero;
        ...
        static THandle Zero { get; } = THandle.FromABI( nint.Zero );
