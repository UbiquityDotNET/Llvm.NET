...
namespace SourceGenerator.Utils
{
    ...
    /// <remarks>
    /// In addition to the potential lack of access to a `nameof` expression there is the problem
    /// of getting a fully qualified namespace name (including any nested types...) for the attribute.
    /// Doing so at compile time  in any sort of consistent form is actually a fairly major PITA.
    /// </remarks>
    ...
    {
        /// <summary>Gets the FullyQualified name of the attribute</summary>
        public string FQN => new StringBuilder(GlobalNamespacePrefix.Length + AttributeTypeName.Length + 1 + NamespaceName.Length)
                           .Append(GlobalNamespacePrefix)
        ...
        public string GeneratedAttributeFileName => new StringBuilder(AttributeTypeName.Length + GeneratedExtension.Length)
                                        ...
                                        .ToString();
        public string ResourceFileName => new StringBuilder(NamespaceName.Length + 1 + AttributeTypeName.Length + ResourceFileExtension.Length)
                                        .Append(NamespaceName)
                                        .Append('.')
                                        .Append(AttributeTypeName)
