...
try
{
    ...
    if(!$ServeDocs)
    {
        ...
        # Delete everything in the docs output except the previous release docs so the new repo has ONLY
        # what is generated by this script plus the previous releases already built (Those are left alone)
        # final commits will include deletion of the older content.
        Write-Information "Cleaning $docsOutputPath"
        Get-ChildItem -Path $docsOutputPath -Exclude 'v*.*.*','.nojekyll', '.git' | remove-item -Recurse -Force -ProgressAction SilentlyContinue
